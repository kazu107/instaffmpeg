...
### Windows で GUI からマスク生成が落ちる場合

`torch` のネイティブ DLL は Tk のウィンドウを作る**前**に読み込む必要があります。
順序が逆になると `transformers` が `torchvision` の C 拡張を読み込む時点で
DLL 解決が衝突し、プロセスがアクセス違反で即死します。

エントリポイントの `main()` は `preload_native_backends()` を先に呼んでこれを
回避しています。`SegFormerMaskGenerator` を GUI 以外から使う場合も、
Tk を作る前に同じ関数を呼んでください。
